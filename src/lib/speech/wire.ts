...
const STYLE_DIRECTIONS: Record<SpeechStyle, string> = {
  "short-expression": "Continue a casual conversation with a friend. Say this short expression compactly and fluidly in one connected thought, with lightly brisk natural pacing. Keep words inside each phrase connected, without a teaching pause or special emphasis on a target expression. Let linking words pass lightly rather than restarting dramatically. End matter-of-factly and easily.",
...
export function buildMimoBody(input: SpeechSynthesisInput, voice: string) {
  ...
  const instruction = [
    ...
    "Speak as a relaxed young adult answering a friend or a podcast interviewer. Keep warm, clear, lightly brisk spontaneous conversational energy. Link unstressed words, use short natural pauses at changes of thought and subtle contrastive emphasis. Keep the voice light and unforced; do not deliberately lower the pitch or imitate a mature broadcaster.",
