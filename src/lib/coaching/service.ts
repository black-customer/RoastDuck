...
import {coachingContextSchema,coachingSubmitSchema,coachingFeedbackSchema,contextCoachingFeedbackSchema,assertCoachingFeedback,type CoachingContext,type CoachingMaterial,type CoachingSubmit,type CoachingFeedback,type CoachingView,type CoachingPracticeEvidence} from './contracts';
import {queueLearningReviewIn,recordCoachingUseEvidenceIn,type TeacherMemory} from './learning-memory';
import {readCoachingPracticeEvidence,linkCoachingPractice} from './practice-evidence';
import type {ContextPracticeTaskRow} from '@/lib/context-practice/contracts';
...
interface Payload {coachingVersion?:'context-coaching-v1';registerProfileVersion?:string;currentTask:{mode:CoachingContext['mode'];questionEn:string;intentZh:string;referenceEnglish?:string;notes?:unknown;origin?:string};latestUserMessage:string;correctionHint:boolean;recentHistory:Array<{role:string;text:string}>;relevantMemories:TeacherMemory[];memoryUsePolicy?:{maxOldIssues:number;onlyWhenRelevant:boolean;absenceIsNotImprovement:boolean};practiceEvidence?:CoachingPracticeEvidence;targets?:Array<{id:string;english:string}>}
interface Meta {context:CoachingContext;assisted:boolean;correctionHint?:boolean;delivery?:'pending'|'completed'|'failed'|'skipped';payload?:Payload;leaseToken?:string|null;leaseUntil?:string|null;errorCode?:string;replyTo?:string;feedback?:CoachingFeedback;reviewJobId?:string|null;runId?:string;practiceEvidence?:CoachingPracticeEvidence}
const metadata=(m:Message)=>parseJson<Meta>(m.metadata_json,{context:{} as CoachingContext,assisted:true});
const contextKey=(c:CoachingContext)=>[c.materialId,c.sentenceId??'',c.mode,c.practiceId,c.sourceSessionId??'',c.rootPracticeId??'',c.relatedTaskId??''].join('|');
...
export function createCoachingService(database:DatabasePort,runtime:RuntimeCalls,memory:MemoryService,platform:Platform){
  ...
  async function material(raw:CoachingContext){
    const c=coachingContextSchema.parse(raw);let m=await platform.loadMaterial(c);
    if(c.relatedTaskId){
      const taskId=c.relatedTaskId,[task]=await database.read(tx=>tx.all<ContextPracticeTaskRow>(sql`SELECT * FROM context_practice_tasks WHERE id=${taskId}`));
      if(!task||task.material_id!==c.materialId||!['ready','completed'].includes(task.status))throw new TrainingError('相关练习尚未准备好或已移除',409,'coaching_related_unavailable');
      if(c.questionId&&c.questionId!==task.question_id)throw new TrainingError('当前题目与相关练习不一致',409,'coaching_question_mismatch');
      m={...m,questionId:task.question_id,questionEn:task.prompt_en,questionZh:task.prompt_zh};
    }
  ...
  async function view(raw:CoachingContext):Promise<CoachingView>{
    ...
    const {evidence}=await database.read(tx=>readCoachingPracticeEvidence(tx,c,platform.now()));
  ...
  async function prepare(raw:CoachingSubmit){
    ...
    return database.write(async tx=>{
      ...
      const {evidence}=await readCoachingPracticeEvidence(tx,c,platform.now());
      const [relatedTask]=c.relatedTaskId?await tx.all<ContextPracticeTaskRow>(sql`SELECT * FROM context_practice_tasks WHERE id=${c.relatedTaskId}`):[];
      const targetIds=relatedTask?JSON.parse(relatedTask.source_sentence_ids_json) as string[]:[];
      ...
      const payload:Payload={coachingVersion:'context-coaching-v1',registerProfileVersion:SPOKEN_REGISTER_VERSION,currentTask:{mode:c.mode,questionEn:m.questionEn,intentZh:c.mode==='answer_independent'?'':sentence?.chinese??m.sentences.map(s=>s.chinese).join('\n'),origin:relatedTask?.origin,
        ...
        relevantMemories:related,memoryUsePolicy:{maxOldIssues:memoryLimit,onlyWhenRelevant:true,absenceIsNotImprovement:true},practiceEvidence:evidence,targets:m.sentences.filter(s=>targetIds.includes(s.id)).slice(0,2).map(s=>({id:s.id,english:s.english}))};
      // A conservative compatibility label only: elapsed time is retained, never a mastery threshold.
      const assisted=c.mode!=='answer_independent'||input.correctionHint||recent.some(r=>r.role==='teacher')||evidence.elapsedMsSinceHelp!==null&&evidence.elapsedMsSinceHelp<24*60*60*1000;
      await linkCoachingPractice(tx,c,evidence,now());
      ...
      await tx.run(sql`INSERT INTO companion_messages(id,thread_id,sequence_no,role,text,status,client_message_id,source_type,source_id,metadata_json,created_at)
        VALUES(${userId},${threadId},${(rows.at(-1)?.sequence_no??0)+1},'user',${input.text},'pending',${input.clientMessageId},'coaching_output',${c.materialId},${JSON.stringify({context:c,assisted,practiceEvidence:evidence,correctionHint:input.correctionHint,delivery:'pending',payload} satisfies Meta)},${now()})`);
  ...
  async function run(input:CoachingSubmit,options:RuntimeCallOptions){
    ...
    try{
      const modern=claimed.payload?.coachingVersion==='context-coaching-v1';
      const result=await runtime.call({role:'companion_response',instructions:await spokenInstructions(platform.loadPrompt,modern?'sentence_coaching.chloe.v3.md':claimed.payload?.registerProfileVersion?'sentence_coaching.chloe.v2.md':'sentence_coaching.chloe.v1.md',claimed.payload?.registerProfileVersion),input:JSON.stringify(claimed.payload),
        schema:modern?contextCoachingFeedbackSchema:coachingFeedbackSchema,schemaName:modern?'sentence_coaching_v2':'sentence_coaching_v1',promptVersion:modern?'sentence-coaching-chloe-v3-context-v1':claimed.payload?.registerProfileVersion?'sentence-coaching-chloe-v2-young-us-v1':'sentence-coaching-chloe-v1',schemaVersion:modern?'sentence-coaching-v2':'sentence-coaching-v1',idempotencyKey:user.id},options);
      ...
      if(modern)assertCoachingFeedback(result.data,payload);
      const jobId=await database.write(async tx=>{
        ...
        for(const [index,bubble] of result.data.messages.entries()){
          ...
          await tx.run(sql`INSERT INTO companion_messages(id,thread_id,sequence_no,role,text,status,client_message_id,source_type,source_id,ai_run_id,metadata_json,created_at)
            VALUES(${replyId},${user.thread_id},${(last?.seq??0)+index+1},'teacher',${bubble.text},'sent',${replyId},'coaching_output',${c.materialId},${result.runId},${JSON.stringify({context:c,assisted:claimed.assisted,practiceEvidence:claimed.practiceEvidence,replyTo:user.id,...(index===0?{feedback:result.data}:{})} satisfies Meta)},${now()}) ON CONFLICT DO NOTHING`);
        }
        ...
        if(c.relatedTaskId&&result.data.extent==='full_answer'&&!input.correctionHint)await tx.run(sql`UPDATE context_practice_tasks SET status='completed',version=version+1,updated_at=${now()} WHERE id=${c.relatedTaskId} AND status='ready'`);
        return jobId;
      });
