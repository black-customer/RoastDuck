...
export function coachingMock(request:StructuredAiRequest<unknown>):unknown{
  ...
  return {verdict:wrong?'incorrect':/[一-鿿]/.test(text)&&!/[A-Za-z]{2}/.test(text)?'uncertain':'natural',extent:p.correctionHint||/I meant|我想改|刚才/.test(text)?'local_correction':'full_answer',
    ...
    findings:wrong?[{kind:'confirmed_error',sourceQuote:'used to live alone',correction:'used to living alone',explanationZh:'表示已经习惯时，be used to 后面接动名词。',memoryKey:'used_to_gerund'}]:[],usedMemoryIds:[],...(request.schemaName==='sentence_coaching_v2'?{targetObservations:(p.targets??[]).map((target:{id:string})=>({sentenceId:target.id,status:'not_observed',sourceQuote:'',reasonZh:'隔离模拟：本次没有独立的目标使用判定。'}))}:{})};
}
