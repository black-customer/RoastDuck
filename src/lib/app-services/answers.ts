...
export interface AnswerDraft {id:string;question_id:string;english_text:string;chinese_text:string;english_unknown:number;version:number;submitted_attempt_id:string|null;source_attempt_id:string|null;kind:'practice'|'independent'|'edit';english_committed_at:string|null;created_at:string;updated_at:string}
...
const patchSchema=z.object({version:z.number().int().nonnegative(),english:z.string().max(16000),chinese:z.string().max(16000),englishUnknown:z.boolean()}).strict();
...
export function createAnswerService(database:DatabasePort,materials:Pick<MaterialService,'prepareIn'>,platform:{now:()=>Date;newId:()=>string;allowMock?:boolean}){
  ...
  async function submit(id:string,version:number){
    return database.write(async tx=>{
      ...
      if((draft.kind!=='independent'&&!draft.chinese_text.trim())||(!draft.english_unknown&&!draft.english_text.trim()))throw new TrainingError("请填写中文原意和英文尝试，或明确选择暂时不会",422,"answer_incomplete");
      if(!draft.chinese_text.trim()&&!draft.english_text.trim())throw new TrainingError('没有可分析的回答，请至少补充中文原意',422,'answer_incomplete');
      ...
      const english=draft.english_unknown?"":draft.english_text;
      await tx.run(sql`INSERT INTO speaking_question_attempts(id,question_id,mode,answer_text,intended_meaning_zh,status,created_at,updated_at)
        VALUES(${attemptId},${question.id},'practice',${english},${draft.chinese_text},'processing',${timestamp},${timestamp})`);
      await tx.run(sql`INSERT INTO practice_submissions(request_id,input_hash,attempt_id) VALUES(${id},${hash(question.id,"practice",english,draft.chinese_text)},${attemptId})`);
      await materials.prepareIn(tx,{sourceType:"ielts_practice",sourceId:attemptId,question:{id:question.id,textEn:question.text,textZh:question.text_zh,part:question.part},mode:"practice",actualAnswer:english,intendedMeaningZh:draft.chinese_text,spokenStyleVersion:SPOKEN_STYLE_VERSION});
