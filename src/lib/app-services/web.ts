...
function buildCompanion(){
  const factory=()=>createAiProvider({mockResolver:request=>{
    if(['sentence_coaching_v1','sentence_coaching_v2','coaching_error_reviewer_v1'].includes(request.schemaName))return coachingMock(request);
    if(request.schemaName==='context_related_question_v1')return relatedQuestionMock(request);
  ...
  return {runtime,materials,memory,contextPractice:createContextPracticeService(nodeDatabase,runtime,{...platform,generationAvailable:()=>getAiHealth().configured}),coaching:createCoachingService(nodeDatabase,runtime,memory,{...platform,loadMaterial:async context=>{
    const material=await getMaterialSentences(context.materialId);
    if(!material||!material.sentences.length||material.sentences.some(s=>!s.english))throw new TrainingError('本次句子材料尚未准备好或已移除，原输出保留',409,'coaching_material_unavailable');
