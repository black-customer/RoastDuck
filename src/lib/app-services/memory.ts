...
export function createMemoryService(database:DatabasePort,runtime:RuntimeCalls,platform:{now:()=>Date;loadPrompt:(name:string)=>string|Promise<string>}){
  ...
  async function relevant(context:string,limit=4){
    ...
    return rows.filter(row=>!sensitiveMemory(row.summary)).map(row=>{
      const content=normalizeKey(row.summary+" "+parseJson<{memoryKey?:string}>(row.detail_json,{}).memoryKey);
      return {row,score:[...terms].filter(term=>content.includes(term)).length+(row.category==="goal"?1:0)};
    }).filter(item=>item.score>0).sort((a,b)=>b.score-a.score||b.row.updated_at.localeCompare(a.row.updated_at)).slice(0,Math.min(4,limit)).map(item=>item.row);
  }
  ...
  return {list,relevant,edit,state,clear,extract,jobs,retry};
}
