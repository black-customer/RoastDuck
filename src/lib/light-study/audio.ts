import { MIMO_TTS_VERSION, VOICE_PRESETS, resolveSpeechStyle, type SpeechStyle, type VoicePresetId } from "@/lib/speech/contracts";
import type { TTSProvider } from "@/lib/tts";
export interface LightAudioInput { text:string; voiceId:VoicePresetId;rate?:number;retryUnknown?:boolean;style?:SpeechStyle }
...
const keyOf=(input:LightAudioInput)=>JSON.stringify([MIMO_TTS_VERSION,input.text,input.voiceId,input.rate??.95,resolveSpeechStyle(input)]);
...
export class LightAudioPlayer {
    ...
    const work=Promise.resolve().then(async()=>{
      ...
      try {
        ...
        const response=await this.fetcher("/api/speech/synthesis",{method:"POST",headers:{"content-type":"application/json"},
          signal,
          body:JSON.stringify({text:input.text,purpose:"example",voice:preset.mimoVoice,accent:preset.accent,rate:input.rate??.95,style:resolveSpeechStyle(input),priority,retryUnknown:input.retryUnknown??false})});
        ...
        if(body.audio.voice!==preset.mimoVoice){this.failures.set(key,'invalid_audio');return null;}
  ...
  async play(input:LightAudioInput) {
    ...
    const audio=this.audioFactory(resource.url);audio.preload="auto";this.audio=audio;
    const voice=resource.voice??VOICE_PRESETS.find(p=>p.id===input.voiceId)!.mimoVoice;
    ...
    audio.onerror=()=>{if(alive()&&this.audio===audio){this.failures.set(keyOf(input),'invalid_audio');fallback();this.releaseResource(keyOf(input));}};
  ...
  dispose(){this.stop();this.disposed=true;this.desired.clear();this.controller.abort();this.pruneResources();}
}
