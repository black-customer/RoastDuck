...
export const prompt = (name: string) => fs.readFileSync(path.join(process.cwd(), "pipeline/prompts", name), "utf8");
...
export async function processMaterial(id: string) {
  ...
  try {
    ...
    await withDbTransaction(async () => {
      ...
      await tx.update(aiJobs).set({ status: "retryable_failure", lastErrorCode: code }).where(eq(aiJobs.id, job.id));
...
export async function publishMaterialItems(material:MaterialRow,input:MaterialInput,analysis:SpeakingAttemptAnalysis){
  return nodeDatabase.write(tx=>publishReviewedItems(tx,material,input,analysis,new Date()));
}
