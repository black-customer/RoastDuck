...
export const selectionMockResolver:MockAiResolver=(request)=>{
  ...
  if(request.schemaName==="four_step_diagnosis_v2") {
    ...
    const natural= !(noun || missing || grammar);
    ...
    return diagnosisSchema.parse({units:[{id:"unit_1",english:answer?[{text:answer,occurrence:0}]:[],chinese:meaning?[{text:meaning,occurrence:0}]:[],
      intentZh:meaning || (noun?"我在外面看到了一个井盖。":grammar?"我已经习惯一个人住了。":"这是原回答的合成中译测试。"),
      status:natural?"natural":missing?"missing":"repair",reasonZh:"隔离回归样例的指定诊断。",gaps}]});
  }
  if(request.schemaName==="four_step_selection_v2") {
    const diagnosis=input.diagnosis as Diagnosis;
    return {approved:true,reasonZh:"独立请求中的合成裁决。",units:diagnosis.units.map((u)=>({unitId:u.id,status:u.status,evidenceQuote:[...u.english,...u.chinese][0].text,reasonZh:"仅用于隔离测试的分类。"})),gaps:diagnosis.units.flatMap((u)=>u.gaps.map((g)=>({gapId:g.id,decision:"train",evidenceQuote:g.evidenceQuote,reasonZh:g.whyNeededZh})))};
  }
  if(request.schemaName==="four_step_material_v2") {
    const diagnosis=input.diagnosis as Diagnosis;
