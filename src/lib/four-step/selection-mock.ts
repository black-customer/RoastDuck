...
export const selectionMockResolver:MockAiResolver=(request)=>{
  ...
  const schemaName=request.schemaName.replace(/_v[3456]$/,'_v2'),spoken=/_v[3456]$/.test(request.schemaName),recall=/_v[456]$/.test(request.schemaName);
  ...
  if(schemaName==="four_step_material_v2") {
    ...
    const sentences=units.map((u,i)=>{
      ...
      return {id:`sentence_${i}`,intentUnitIds:[u.id],english,...(input.source.teachingVersion?{teaching:{version:'sentence-teaching-v1',overviewZh:'原创隔离夹具：先表达主体，再补充动作和场景。',parts:[{cueZh:u.intentZh,quoteEn:english,explanationZh:'仅供交互与结构回归：这段英文承接夹具指定的完整中文意思，不用于私人内容教学。',pattern:'',examples:[],alternatives:[],contrastZh:''}]}}:{})};
    });
