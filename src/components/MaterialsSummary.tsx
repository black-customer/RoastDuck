...
export function MaterialsSummary({analysis,materialId,questionId,sourceId,originalEnglish,originalChinese,initialLesson}:{analysis:SpeakingAttemptAnalysis;materialId:string;questionId?:string;sourceId:string;originalEnglish:string;originalChinese:string;onStrengthen:()=>void;initialLesson?:MaterialLesson|null}){
  ...
  return <div className="material-result"><section className="page-panel material-summary"><h1>把你想说的话，放回完整语境里</h1><p>中英对照，按需揭晓与求助。可以随时暂停、换题，不用评分才能继续。</p>
    {analysis.corrections[0]&&<p className="material-feedback">{analysis.corrections[0].reasonZh}</p>}
    {lesson?.sentences.length?<><p>{lesson.sentences.length} 句已确认的自然表达</p><Link className="primary-button" href={`/sentence-study?scope=material&id=${encodeURIComponent(materialId)}&mode=learn`}>开始整题学习</Link></>:<p role="status">{lesson===undefined?'正在读取已审核的句子版本…':'明确的句子还在整理或等待原意确认，原回答已保留。'}</p>}
    ...
    {naturalVersion&&<section className="page-panel material-natural"><div className="material-natural-heading"><h2>{needsAttention.length?'已确认的自然表达':'你的自然表达'}</h2>{!needsAttention.length&&<SpeakButton text={naturalVersion} style={questionId?'ielts-answer':'daily-conversation'} label="播放自然表达全文" playbackMode="natural"/>}</div>{needsAttention.length>0&&<p className="material-feedback">仅含已确认内容，另有细节待补充。这些句子不是完整回答；可以单句学习、试听。</p>}<p lang="en" className="material-natural-text">{naturalVersion}</p><details open={sentencesOpen} onToggle={event=>setSentencesOpen(event.currentTarget.open)}><summary>逐句试听</summary>{sentencesOpen&&(needsAttention.length?lesson?.sentences.map((sentence,index)=><div key={sentence.id}><p lang="en" className="material-natural-text">{sentence.english}</p><SpeakButton text={sentence.english} style={questionId?'ielts-answer':'daily-conversation'} label={`试听第 ${index+1} 句`} playbackMode="natural"/></div>):<NaturalVersionPlayer naturalVersion={naturalVersion} style={questionId?'ielts-answer':'daily-conversation'}/>)}</details></section>}
