...
import {getTTS,getStoredVoicePreference} from '@/lib/tts';
...
export function SpeechSettings(){
  ...
  return <section className="settings-section"><div className="settings-section-copy"><h2>声音与 AI 服务</h2><p>默认自然美式英语。高质量声音不可用时自动备用，并显示真实音源。</p></div><div className="settings-field"><strong>MiMo 示范声音</strong><p>{status(speech)}</p><button type="button" className="secondary-button" onClick={()=>{setProvider('mimo');setSecret('');}}>配置自己的 MiMo Key</button></div><div className="settings-field"><strong>DeepSeek 表达分析</strong><p>{text?.configured?'已配置（不代表本次连接一定成功）':'未配置；已有材料仍可学习'}</p><button type="button" className="secondary-button" onClick={()=>{setProvider('deepseek');setSecret('');}}>配置自己的 DeepSeek Key</button></div>
    ...
    <label className="settings-field">示范声线<select value={voice} onChange={e=>{const selected=e.target.value as VoicePresetId;setVoice(selected);getTTS().setVoice(selected);}}>{VOICE_PRESETS.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select></label>
