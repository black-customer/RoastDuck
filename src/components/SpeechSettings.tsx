...
import type {SpeechRole} from '@/lib/speech/contracts';
...
export function SpeechSettings(){
  const ownerId=useId(),[audio,setAudio]=useState<LightAudioState|null>(null),[sampleRole,setSampleRole]=useState<SpeechRole>('learning');
  ...
  const sample=(retryUnknown=false,role:SpeechRole=sampleRole)=>{setSampleRole(role);getTTS().speak(role==='teacher'?"That sounds natural. Tell me a little more about it.":"I'd like to reschedule my test.",{voice:getSpeechPreferences(role).voice,lang:getSpeechPreferences(role).accent,role,ownerId,retryUnknown,playbackMode:'natural',onState:setAudio,onEnd:()=>void load().catch(()=>undefined),onError:()=>setMessage('声音暂不可用，可以继续文字学习')});};
  ...
  return <section className="settings-section"><div className="settings-section-copy"><h2>声音与 AI 服务</h2><p>默认年轻美式男声。自然跟练优先MiMo，快捷试听可使用系统声音；始终显示实际音源。</p></div><div className="settings-field"><strong>MiMo 示范声音</strong><p>{status(speech)}</p><button type="button" className="secondary-button" onClick={()=>{setProvider('mimo');setSecret('');}}>配置自己的 MiMo Key</button></div><div className="settings-field"><strong>{text?.displayName??'DeepSeek V4.1 Flash'} 表达分析</strong><p>{text?.configured?'已配置（不代表本次连接一定成功）':'未配置；已有材料仍可学习'}</p><button type="button" className="secondary-button" onClick={()=>{setProvider('deepseek');setSecret('');}}>配置自己的 DeepSeek Key</button></div>
    ...
    <div className="settings-field"><h3>学习示范</h3><SpeechPreferences role="learning"/></div>
    <div className="settings-field"><h3>Chloe 老师</h3><SpeechPreferences role="teacher"/></div>
    <p className="quiet">学习默认 Milo / 美式，老师默认 Chloe / 美式；声线与口音分别保存，两组共用播放速度。</p>
    <div className="row-actions"><button type="button" className="secondary-button" onClick={()=>sample(false,'learning')}>试听学习声音</button><button type="button" className="secondary-button" onClick={()=>sample(false,'teacher')}>试听老师声音</button><button type="button" onClick={()=>getTTS().stop(ownerId)}>停止</button><button type="button" onClick={()=>void load().catch(()=>setMessage('状态读取失败，请重试'))}>刷新状态</button></div>
