...
export function ExpressionSummary({summary,scope}:{summary:Summary;scope:LightScope}){
  ...
  return <section className={styles.summary} aria-label="表达学习概况">
    ...
    <div className={styles.studyActions}>
      {summary.new>0?<Link className={summary.due>0?'secondary-button':'primary-button'} href={`/light-study?extension=1&${query}&mode=learn`}>学新表达（{summary.new}）</Link>:<button type="button" className="secondary-button" disabled>学新表达（0）</button>}
      {summary.due>0?<Link className="primary-button" href={`/light-study?extension=1&${query}&mode=review`}>复习到期表达（{summary.due}）</Link>:<button type="button" className="secondary-button" disabled>复习到期表达（0）</button>}
      {summary.eligibleTotal>0&&<Link className={styles.quickLink} href={`/quick-review?extension=1&${query}`}>快速回顾</Link>}
    </div>
