...
export function QuickReview({scope}:{scope:LightScope}){
  ...
  return <div className={`page-content ${styles.page}`}><Link className={styles.back} href={`/expressions?${query}`}>返回{expressionScopeTitle(scope)}</Link><header className={styles.header}><h1>快速回顾</h1><p>{expressionScopeTitle(scope)} · 先看中文，按需揭晓英文和播放。这里只读浏览，不记录学习、自评或复习进度。</p></header>
    {error?<div className="error-banner" role="alert">{error}<button onClick={()=>setRetry(value=>value+1)}>重新读取</button></div>:loading?<p role="status">正在读取表达…</p>:items.length?<div className={styles.list}>{items.map(item=><QuickReviewRow key={item.itemId} item={item}/>)}</div>:<div className={styles.empty}><p>这个范围暂时没有可回顾的表达；暂不学的项目可在我的表达中恢复。</p><Link className="secondary-button" href={`/expressions?${query}`}>查看我的表达</Link></div>}
  </div>;
}
