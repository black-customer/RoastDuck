...
export function SpeakButton({
  ...
  return (
    ...
    </button>{state?.message&&<span className="speech-control-status" role="status">{state.message}</span>}
    {playbackMode==='natural'&&['loading','error'].includes(state?.phase??'')&&state?.errorCode!=='local_voice_unavailable'&&<button type="button" onClick={()=>play(false,'quick')}>先听快捷声音</button>}
    {state?.errorCode==='local_voice_unavailable'&&<button type="button" onClick={()=>play(false,'natural')}>准备自然声音</button>}
    {state?.phase==='playing'&&<button type="button" onClick={()=>getTTS().stop(ownerId,true)}>停止</button>}
    {['result_unknown','invalid_audio'].includes(state?.errorCode??'')&&<button type="button" onClick={()=>{if(window.confirm('重新合成这一条声音可能再次使用 MiMo 余额。确认继续？'))play(true);}}>重新生成此声音</button>}</span>
  );
}
