...
export function SpeakButton({
  ...
  const play=(retryUnknown=false)=>getTTS().speak(text,{rate,lang,ownerId,retryUnknown,onState:state=>{if(active.current)setState(state);}});
  const cls = size === "lg"
  ...
  return (
    <span className="speech-control"><button type="button" aria-label={label} title={label} className={cls} onClick={() => play()}>
