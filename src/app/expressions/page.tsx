...
export default async function ExpressionsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <div className={`page-content ${styles.page}`}><header className={styles.header}><h1>{expressionScopeTitle(scope)}</h1><p>来自你自己的回答和对话。同一个意思共享学习记录，随时可以查看、复习或调整。</p></header>
    <nav className={styles.tabs} aria-label="个人表达集合"><Link href="/expressions?extension=1&scope=collection&id=ielts" aria-current={scope.type==='collection'&&scope.id==='ielts'?'page':undefined}>我的雅思表达</Link><Link href="/expressions?extension=1&scope=collection&id=free_talk" aria-current={scope.type==='collection'&&scope.id==='free_talk'?'page':undefined}>我的对话表达</Link></nav>
