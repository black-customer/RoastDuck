...
export default async function SentenceStudyPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const input:SentenceCreate={scope:parsed.success?parsed.data:{type:'all'},mode:params.mode==='review'?'review':'learn',clientRequestId:crypto.randomUUID(),selection:params.selection==='random'?'random':params.selection==='due'?'due':'scope',experienceVersion:params.extension==='guided'?'guided-reveal-v1':'context-workspace-v1'};
