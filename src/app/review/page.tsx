...
import {LegacyModeGate} from '@/components/sentence-study/LegacyModeGate';
export const dynamic = "force-dynamic";
export default async function Page({searchParams}:{searchParams:Promise<{extension?:string}>}) {
  if((await searchParams).extension!=='1')return <LegacyModeGate title="原复习与强化" href="/review?extension=1"/>;
  ...
  return <div className="page-stack"><LightStudy scope={{type:'all'}} initialMode="review"/><details><summary>可选四步复习</summary><p>需要更严格的提取训练时再使用，不影响轻松学的复习进度。</p>
    {review.length ? review.map((source) => <section className="card" key={source.materialId}>
      ...
      <Link className="primary-button" href={`${source.href}${source.href.includes('?')?'&':'?'}extension=1`}>{source.active ? "继续这次复习" : "开始本次复习"}</Link>
