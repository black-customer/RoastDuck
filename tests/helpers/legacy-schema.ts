...
export async function migrationHistory(client:Client,maxVersion=36){
...
export async function removePostV25Schema(client:Client,url:string):Promise<void>{
  ...
  if(before.map(row=>row.version).join(',')!==Array.from({length:36},(_,index)=>index+1).join(','))throw new Error('旧结构夹具要求完整且连续的 v36 起始库');
  const reverseStatements=[V36_DDL,V35_DDL,V34_DDL,V33_DDL,V32_DDL,V31_DDL,V30_DDL,V29_DDL,V28_DDL,V27_DDL,V26_DDL].flatMap(statements=>[...statements].reverse().map(statement=>{
