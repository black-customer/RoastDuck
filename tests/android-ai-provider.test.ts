...
const response={id:"synthetic-response",model:"deepseek-v4-flash",status:"completed",output:[{type:"message",content:[{type:"output_text",text:'{"answer":"Hello"}'}]}]};
it("native bridge receives a structured request and receipt ID but no key, URL or headers",async()=>{
  ...
  expect(JSON.parse(request.mock.calls[0][0].body)).toMatchObject({model:"deepseek-v4-flash",input:"hello"});
