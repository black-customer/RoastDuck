...
test('整题中文先行，揭晓/切句不等网络，三档评分后停留，完成后可更正评分',async({page,request})=>{
  await setup(page);await page.goto('/sentence-study?extension=guided&scope=question&id=sentence-e2e-new&mode=learn');
...
test('暂停/刷新恢复同一题，句子完整呈现；窄屏和旧模式隔离',async({page})=>{
  await setup(page);await page.goto('/sentence-study?extension=guided&scope=question&id=sentence-e2e-next&mode=learn');await expect(page.getByRole('heading',{name:'我想弹吉他。'})).toBeVisible();
...
test('句子复习只装载到期内容，可选输出独立时没有旧答案',async({page,request})=>{
  await setup(page);await page.goto('/sentence-study?extension=guided&scope=question&id=sentence-e2e-review&mode=review');await expect(page.getByRole('button',{name:'看自然表达与讲解',exact:true})).toBeVisible();
  ...
  await expect(page.getByRole('heading',{name:/本次(句子学习|到期复习)已完成/})).toBeVisible();await page.getByRole('button',{name:'不看提示，独立回答',exact:true}).click();await expect(page.getByRole('textbox',{name:'你的英文表达',exact:true})).toBeVisible();await expect(page.getByText('我想喝点水。',{exact:true})).toHaveCount(0);await expect(page.getByText('I want to drink some water.',{exact:true})).toHaveCount(0);
...
test('连续三十句的浏览器揭晓和切句满足即时响应预算',async({page})=>{
  await setup(page);await page.goto('/sentence-study?extension=guided&scope=question&id=sentence-e2e-performance&mode=learn');await expect(page.getByRole('heading',{name:'我想阅读第1章。',exact:true})).toBeVisible();
...
test('可选本地再练刷新后保留当前输入，跳过不自动请求老师',async({page})=>{
  await setup(page);await page.goto('/sentence-study?extension=guided&scope=question&id=sentence-e2e-output&mode=learn');
