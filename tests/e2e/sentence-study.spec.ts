...
test('整题中文先行，揭晓/切句不等网络，三档均继续，完成后可更正评分',async({page,request})=>{
  await setup(page);await page.goto('/sentence-study?scope=question&id=sentence-e2e-new&mode=learn');
  await expect(page.getByRole('heading',{name:'我想做早餐。',exact:true})).toBeVisible();await expect(page.getByRole('textbox')).toHaveCount(0);await expect(page.getByText('I want to make breakfast.',{exact:true})).toHaveCount(0);await expect(page.getByText(/秒后揭晓/)).toHaveCount(0);
  ...
  for(const [index,label] of ['脱口而出','想了一会儿','没想出来'].entries()){
    await page.getByRole('button',{name:'看自然表达',exact:true}).click();await expect(page.locator('article [lang="en"]').first()).toBeVisible();
...
test('暂停/刷新恢复同一题，句子完整呈现；窄屏和旧模式隔离',async({page})=>{
  ...
  for(const width of [1280,390,320]){await page.setViewportSize({width,height:900});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await expect(page.getByRole('button',{name:'看自然表达',exact:true})).toBeInViewport();if(width!==320)await page.screenshot({path:`test-results/visual/sentence-prompt-${width}.png`,fullPage:true});}
  await page.getByRole('button',{name:'看自然表达',exact:true}).click();await page.getByRole('button',{name:/^脱口而出/}).click();await expect(page.getByRole('heading',{name:'我想听音乐。'})).toBeVisible();await page.getByRole('button',{name:'暂停学习',exact:true}).click();
...
test('句子复习只装载到期内容，可选输出独立时没有旧答案',async({page,request})=>{
  await setup(page);await page.goto('/sentence-study?scope=question&id=sentence-e2e-review&mode=review');await expect(page.getByRole('button',{name:'看自然表达',exact:true})).toBeVisible();
  for(let i=0;i<2;i++){await page.getByRole('button',{name:'看自然表达',exact:true}).click();await page.getByRole('button',{name:/^脱口而出/}).click();}
...
test('连续三十句的浏览器揭晓和切句满足即时响应预算',async({page})=>{
  ...
  const samples=await page.evaluate(async()=>{
    ...
    for(let i=1;i<=30;i++){
      ...
      const reveal=Array.from(document.querySelectorAll('button')).find(b=>b.textContent?.trim()==='看自然表达');if(!reveal)throw new Error('Missing reveal');
      let start=performance.now();reveal.click();await until(()=>!!document.querySelector('article [lang="en"]'));readings.push(performance.now()-start);
      await new Promise<void>(resolve=>requestAnimationFrame(()=>resolve()));
      ...
      start=performance.now();rate.click();await until(()=>i===30?Array.from(document.querySelectorAll('h1')).some(h=>h.textContent==='本次句子学习已完成'):Array.from(document.querySelectorAll('h1')).some(h=>h.textContent===`我想阅读第${i+1}章。`));ratings.push(performance.now()-start);
    }
...
test('可选再表达一次刷新后保留当前练习、输入和下一句位置',async({page})=>{
  ...
  await page.getByRole('checkbox',{name:'再表达一次（可选）',exact:true}).check();
  await page.getByRole('button',{name:'学习设置',exact:true}).click();
  await page.getByRole('button',{name:'看自然表达',exact:true}).click();
  await page.getByRole('button',{name:/^没想出来/}).click();
  await page.getByRole('textbox',{name:'你的英文表达',exact:true}).fill('I want to cook dinner.');
  await page.reload();
  await expect(page.getByRole('textbox',{name:'你的英文表达',exact:true})).toHaveValue('I want to cook dinner.');
  await expect(page.getByText('我想洗碗。',{exact:true})).toHaveCount(0);
  await page.getByRole('button',{name:'关闭表达练习',exact:true}).click();
