...
describe("Chloe API", () => {
  it("创建线程、发送混合消息并可恢复读取", async () => {
    const created = await threadRoute.POST(request("http://127.0.0.1/api/companion/threads", "POST", { scopeType: "general", scopeId: null }));
    expect(created.status).toBe(201);
    const thread = (await created.json()).thread as { id: string; messages: unknown[] };
    expect(thread.messages).toHaveLength(1);

    const clientMessageId = randomUUID();
    const sent = await messageRoute.POST(request("http://127.0.0.1/messages", "POST", { clientMessageId, text: "I like coffee，也想提高口语。", inputLanguage: "mixed", messageKind: "text" }), { params: Promise.resolve({ id: thread.id }) });
    expect(sent.status).toBe(200);
    expect((await sent.json()).thread.messages).toHaveLength(4);
    const loaded = await messageRoute.GET(request("http://127.0.0.1/messages", "GET"), { params: Promise.resolve({ id: thread.id }) });
  ...
  it("拒绝非法作用域，并支持记忆修改和清空", async () => {
    const invalid = await threadRoute.POST(request("http://127.0.0.1/api/companion/threads", "POST", { scopeType: "gap", scopeId: null }));
    ...
    const patched = await memoryRoute.PATCH(request("http://127.0.0.1/api/companion/memories", "PATCH", { id: "memory_api", summary: "用户正在准备雅思口语。" }));
    expect(patched.status).toBe(200);
    expect((await patched.json()).memory.summary).toContain("雅思口语");
    expect(await service.listCompanionMemories()).toHaveLength(1);
    const cleared = await memoryRoute.DELETE(request("http://127.0.0.1/api/companion/memories", "DELETE", { all: true }));
