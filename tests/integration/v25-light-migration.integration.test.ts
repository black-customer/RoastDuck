...
// This single integration case initializes v35, reconstructs physical v24, then
// reapplies migrations and settles a review. Its budget is not a click-latency SLA.
it("real v24-shaped stored review upgrades to V1, keeps queue/reveal/progress and can finish",async()=>{
  ...
  try{
    ...
    expect((await migrationHistory(client)).map(row=>row.version)).toEqual(Array.from({length:35},(_,index)=>index+1));
