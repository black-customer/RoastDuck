...
## 当前测试
v0.4.0 CONTEXT_WORKSPACE：完整语境与正式目标分离、接触24小时首次回想、幂等评分/恢复、句子偏好与内容隔离；完整回答身份、合成MediaRecorder、分片上传/丢回执、原文件Range/删除、历史A/B、双角色声音及v2认证备份。录音测试不用真实麦克风；大于64MiB备份检验字节哈希。旧guided-reveal浏览器回归必须显式extension=guided，不把兼容测试当新默认路线。
v0.3.0 GUIDED_REVEAL：逐词游标/遮挡与键盘、草稿回执与复制窗口锁、自评停留/再练/更正/完成、准确教学锚点和独立审核、明确partialApproval隔离、版本父链/精确替換并发、旧v5任务/新v6教学合同。既有材料作者与Reviewer记录只在私有目录，不能用Mock教学占位替代内容完成。
