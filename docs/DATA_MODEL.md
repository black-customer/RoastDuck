...
## 迁移与隐私
v37只增light_study_events(learning_item_id)与(created_at)索引，服务首页未完成探针；无数据变更，先备份后应用。
...
Web业务备份（WEB_BACKUP_TABLES）覆盖v2学习管线、个人Gap诊断台账、V3提取历史、个人Chunk/句子、题目练习状态、口语会话、回答导入归档与原声元数据；缺失表会在换机恢复时丢进度或产生悬空收件箱引用，新增个人数据表必须同步登记进备份清单。备份不含Key、私人正文与音频文件本体（音频走含媒体备份）。
