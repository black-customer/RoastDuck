...
中英文对齐按意思，而非逐段分两遍：同一意思的英文尝试和中文解释归到同一个unit，english/chinese/raw可以引用前后不连续的多个片段。先用中文确认完整意图，再判断配对英文是否自然；英文有错就repair，不能把重复的中文另建missing或只因重复就标natural。中文新增且没有英文依据的细节才单独missing。口头重复、改口也保留raw覆盖，但只呈现一次完整意思，不重复制造句子或目标。
